...
package com.kubeiwu.baseclass.view;
...
/**
 * Copy a the static library support version of the framework's {@link android.app.ListFragment}, but then targeted for a {@link GridView}.
 */
public class GridFragment extends Fragment {
	 ...
	 * 
	 * <p>
	 * If you are overriding this method with your own custom content, consider including the standard layout {@link android.R.layout#list_content} in your layout file, so that you continue to retain all of the standard behavior of GridFragment. In particular, this is currently the only way to have the built-in indeterminant progress state be shown.
	 */
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		gframe.addView(tv, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT));

		GridView gv = new GridView(context);
		gv.setId(android.R.id.list);
		gv.setDrawSelectorOnTop(false);
		gframe.addView(gv, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT));

		root.addView(gframe, new FrameLayout.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT));
	...
	/**
	 * This method will be called when an item in the grid is selected. Subclasses should override. Subclasses can call getGridView().getItemAtPosition(position) if they need to access the data associated with the selected item.
	 * 
	 * @param gv
	...
	/**
	 * The default content for a GridFragment has a TextView that can be shown when the grid is empty. If you would like to have it shown, call this method to supply the text it should use.
	 */
	 ...
	 * 
	 * <p>
	 * Applications do not normally need to use this themselves. The default behavior of GridFragment is to start with the grid not being shown, only showing it once an adapter is given with {@link #setListAdapter(ListAdapter)}. If the grid at that point had not been shown, when it does get shown it will be do without the user ever seeing the hidden state.
	 * 
	 * @param shown
	 *            If true, the grid view is shown; if false, the progress indicator. The initial value is true.
	 */
	...
	/**
	 * Like {@link #setGridShown(boolean)}, but no animation is used when transitioning from the previous state.
	 */
